...
export default function SellerPayoutSettingsForm({ isOpen, onClose, userId, sellerProfile, onSaved }: Props) {
  ...
  useEffect(() => {
    if (!isOpen) return;
    const m = sellerProfile?.payout_method || 'bank';
    const d = sellerProfile?.payout_details || {};
    setMethod(m);
    setUpiId(d.upi_id || '');
  ...
  const saveSettings = async () => {
    ...
    try {
      ...
      if (method === 'upi') {
        ...
        } else if (sellerProfile?.payout_details?.account_number) {
          payoutDetails.account_number = sellerProfile.payout_details.account_number;
        }
  ...
  const isSaved = !!(sellerProfile?.payout_method);
        ...
        <div className="p-6 border-b border-[rgba(0,0,0,0.08)]">
          ...
          <div className="flex items-start justify-between">
            <div>
              ...
              <p className="text-[12px] text-[#6E6E73] mt-1">How you receive your store earnings</p>
            </div>
