...
import {
  ...
  ChevronLeft, ChevronRight, Info, ShoppingBag,
} from 'lucide-react';
import { toast } from 'sonner';
import { createClient } from '@/lib/supabase/client';
import SellerRevenueChart from './SellerRevenueChart';
import ProductEarningsBreakdown from './ProductEarningsBreakdown';
import SellerTransactionRow from './SellerTransactionRow';
import SellerPayoutSettingsForm from './SellerPayoutSettingsForm';
...
function exportCSV(transactions: Transaction[]) {
  const headers = ['Date', 'Description', 'Product', 'Qty', 'Gross Amount', 'Your Earnings', 'Status'];
  ...
  a.href = url;
  a.download = `slate-store-earnings-${new Date().toISOString().slice(0, 10)}.csv`;
  a.click();
...
export default function PayoutsPageClient({
  sellerProfile, transactions, productEarnings, monthlyData, stats, userId,
}: Props) {
  const supabase = createClient();
  const [profile, setProfile] = useState<any>(sellerProfile);
  ...
  const filtered = useMemo(() => {
    ...
    if (typeFilter === 'Sales') result = result.filter(t => t.type === 'sale');
    if (typeFilter === 'Payouts') result = result.filter(t => t.type === 'payout');
    if (search.trim()) {
      const q = search.toLowerCase();
      result = result.filter(t => t.description.toLowerCase().includes(q) || t.product_name?.toLowerCase().includes(q));
    }
    return result;
    ...
    <div className="mx-auto max-w-6xl px-6 py-8 font-[DM_Sans]">
      ...
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="mt-1 text-[13px] text-[#6E6E73]">Track your store earnings and payouts</p>
        </div>
        <button
      ...
      <MacCard className="mb-6">
        <TitleBar>Store Revenue</TitleBar>
        <div className="p-6 flex items-end justify-between flex-wrap gap-4">
          <div>
            ...
            <div className="flex items-center gap-1.5 mt-1">
              <Info className="w-3 h-3 text-[#AEAEB2]" />
              <span className="text-[12px] text-[#AEAEB2]">
                You keep {stats.commissionRate}% · Platform fee: {100 - stats.commissionRate}%
              </span>
            </div>
          </div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
        {[
          ...
          {
            icon: CheckCircle2,
            value: `₹${stats.netEarnings.toLocaleString('en-IN')}`,
            label: `Your Earnings (${stats.commissionRate}%)`,
      ...
      {/* ─── REVENUE CHART ─── */}
      <SellerRevenueChart monthlyData={monthlyData} />

      {/* ─── TWO COLUMN ─── */}
      {transactions.length > 0 ? (
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-6">
          {/* LEFT: Per-Product */}
          <ProductEarningsBreakdown productEarnings={productEarnings} />
          ...
          <MacCard className="h-full flex flex-col">
            ...
            <div className="flex-1 divide-y divide-[rgba(0,0,0,0.05)] overflow-auto">
              {recentFiltered.length === 0 ? (
                ...
                recentFiltered.map(t => <SellerTransactionRow key={t.id} transaction={t} />)
              )}
            </div>
          </MacCard>
        </div>
        ...
        <MacCard className="mb-6">
          ...
          <div className="py-24 flex flex-col items-center justify-center text-center">
            ...
            <p className="text-[14px] text-[#6E6E73] mt-2 max-w-xs">
              Earnings will appear here once customers purchase your products.
            </p>
            <a
        ...
        <MacCard className="mb-6">
          ...
          <div className="px-5 py-3 border-b border-[rgba(0,0,0,0.06)] flex items-center gap-3 flex-wrap">
            ...
            <select
              ...
              className="border border-[rgba(0,0,0,0.1)] rounded-xl px-3 py-1.5 text-[12px] text-[#1D1D1F] bg-[#F5F5F7] focus:outline-none"
            >
              {['All Types', 'Sales', 'Payouts'].map(r => <option key={r}>{r}</option>)}
            </select>
          ...
          {/* Table Header */}
          <div className="bg-[#F5F5F7] px-5 py-3 grid grid-cols-[100px_1fr_60px_80px_80px_80px] gap-4">
            {['Date', 'Product', 'Qty', 'Gross', 'Your Share', 'Status'].map(h => (
            ...
            {paginated.length === 0 ? (
              ...
              paginated.map(t => {
                ...
                return (
                  <div
                    key={t.id}
                    className="px-5 py-3 grid grid-cols-[100px_1fr_60px_80px_80px_80px] gap-4 border-b border-[rgba(0,0,0,0.05)] hover:bg-[#F5F5F7] transition-colors items-center"
                  >
                    <span className="text-[12px] text-[#6E6E73]">{format(new Date(t.created_at), 'dd MMM yyyy')}</span>
                    <span className="text-[12px] font-medium text-[#1D1D1F] truncate">{t.product_name || t.description}</span>
                    <span className="text-[12px] text-[#6E6E73]">{t.quantity}</span>
              ...
              {profile?.payout_method ? (
                <div className="mt-4 text-[12px] text-[#6E6E73]">
                  ...
                  <span className="font-semibold text-[#1D1D1F] capitalize">
                    {profile.payout_method === 'bank' ? 'Bank Transfer' : profile.payout_method === 'upi' ? 'UPI' : 'PayPal'}
                  </span>
