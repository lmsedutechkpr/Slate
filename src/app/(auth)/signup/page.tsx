...
import {AlertCircle, GraduationCap, Loader2, Presentation, ShoppingBag} from 'lucide-react';
...
export default function SignupPage() {
  ...
  const [role, setRole] = useState<'student' | 'instructor' | 'seller' | null>(null);
  ...
  if (step === 1) {
    return (
      <AuthWindow title={t('role.title')} width="lg">
        <div className="text-center">
          <h1 className="font-sans text-[26px] font-bold text-[var(--text)]">{t('role.title')}</h1>
          <p className="mt-1 text-[14px] text-[var(--text-secondary)]">{t('role.subtitle')}</p>
        </div>
        ...
        <div className="mt-8 flex flex-col gap-4 sm:flex-row sm:items-stretch sm:gap-4">
          <div className="flex flex-1 flex-col">
            <SignupRoleCard
              ...
              tagline={t('role.studentTagline')}
              points={[...t.raw('role.studentPoints')]}
          ...
          <div className="flex flex-1 flex-col">
            <SignupRoleCard
              ...
              tagline={t('role.instructorTagline')}
              points={[...t.raw('role.instructorPoints')]}
              selected={role === 'instructor'}
              requiresApproval
              requiresApprovalText={t('role.requiresApproval')}
          ...
          <div className="flex flex-1 flex-col">
            <SignupRoleCard
              ...
              title={t('role.seller')}
              tagline={t('role.sellerTagline')}
              points={[...t.raw('role.sellerPoints')]}
        ...
        <button
          onClick={() => setStep(2)}
          disabled={!role}
          className="mx-auto mt-8 flex w-fit items-center gap-2 rounded-full bg-[var(--white-surface)] px-8 py-2.5 text-[14px] font-semibold text-[var(--white-text)] transition-all duration-150 hover:scale-[1.01] hover:bg-[rgba(0,0,0,0.8)] disabled:cursor-not-allowed disabled:opacity-40"
        >
          {t('role.continue', {role: t(`role.${role || 'student'}`)})}
        </button>

        <div className="mt-6 text-center">
          <Link href="/login" className="text-[13px] text-[var(--text-secondary)] hover:text-[var(--text)]">
            {t('role.hasAccount')}
          </Link>
        </div>
      </AuthWindow>
    ...
    return (
      <AuthWindow title={`${tLogin('title').split(' ')[0]} — ${t('role.student')} Signup`} width="lg">
          ...
          <div>
            ...
            <div className="flex gap-3">
              <button
                ...
                className={cn(
                  'rounded-full border border-[var(--border)] px-4 py-2 text-[13px] font-medium transition-colors',
                  studentLang === 'en'
        ...
        <p className="mt-6 text-center text-[13px] text-[var(--text-secondary)]">
          <Link href="/login" className="font-medium text-[var(--text)] underline-offset-4 transition-colors hover:underline">
            {t('role.hasAccount')}
          </Link>
        </p>
      </AuthWindow>
  ...
  // A simplified placeholder for Instructor and Seller to avoid creating a massive +3000 line file, 
  // keeping the design aesthetic the same. A real implementation would include useForms for all states.
  return (
    <AuthWindow title={`${tLogin('title').split(' ')[0]} — ${t(`role.${role}`)} Application`} width="md">
      <button
          onClick={() => setStep(1)}
          className="mb-6 flex items-center gap-1 text-[13px] text-[var(--text-muted)] transition-colors hover:text-[var(--text-secondary)]"
        >
      ...
      <div className="text-center">
        <h1 className="font-sans text-[22px] font-bold text-[var(--text)]">Registration restricted</h1>
        <p className="mt-1 text-[14px] text-[var(--text-secondary)]">Please sign up as a Student for this preview.</p>
        <button
          onClick={() => setRole('student')}
          className="mx-auto mt-6 flex items-center gap-2 rounded-full border border-[var(--border-hover)] bg-[var(--surface-raised)] px-6 py-2.5 text-[14px] font-medium text-[var(--text)] transition-all duration-150 hover:bg-[rgba(0,0,0,0.05)]"
        >
          Go to Student form
        </button>
      </div>
