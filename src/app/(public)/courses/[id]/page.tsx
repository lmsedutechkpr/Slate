...
export default async function CourseDetailPage({
  ...
  if (user) {
    ...
    if (profile?.role === 'student') {
      redirect(`/student/courses/browse/${id}`)
    }
    if (profile?.role === 'instructor') {
      redirect(`/instructor/courses/browse/${id}`)
    }
    if (profile?.role === 'seller') {
      redirect(`/seller/browse/${id}`)
    }
  }
  ...
  if (user) {
    const [enrollResp, wishResp, reviewResp] = await Promise.all([
    ...
    enrollment = enrollResp.data;
    isWishlisted = !!wishResp.data;
  ...
  return (
    <div className="min-h-screen bg-[#0A0A0A] text-white flex flex-col font-sans">
      ...
      <main className="flex-1 w-full flex flex-col pt-[72px]">
          ...
          <div className="flex flex-col lg:flex-row gap-12 relative">
            ...
            <div className="flex-1 min-w-0 pb-20 lg:pb-0">
              ...
              {whatYouLearn.length > 0 && (
                <div className="bg-[#111111] border border-[rgba(255,255,255,0.08)] rounded-2xl p-6 lg:p-8">
                  <h2 className="font-sans font-bold text-[22px] text-white mb-6">What you'll learn</h2>
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-4">
                    {whatYouLearn.map((item: string, idx: number) => (
                      <div key={idx} className="flex items-start gap-3">
                        ...
                        <span className="text-[14px] text-[#8E8E93] leading-relaxed">{item}</span>
                      </div>
                    ))}
                  </div>
                </div>
              ...
              {requirements.length > 0 && (
                <div className="mt-12">
                  <h2 className="font-sans font-bold text-[22px] text-white mb-5">Requirements</h2>
                  <ul className="flex flex-col gap-3">
                    {requirements.map((req: string, idx: number) => (
                      <li key={idx} className="flex items-start gap-3">
                        <div className="w-[6px] h-[6px] rounded-full bg-[#48484A] mt-2 shrink-0" />
                        <span className="text-[14px] text-[#8E8E93] leading-relaxed">{req}</span>
                      </li>
                    ))}
                  </ul>
                </div>
        ...
        <div className="lg:hidden fixed bottom-0 left-0 right-0 z-50 bg-[rgba(17,17,17,0.92)] backdrop-blur-xl border-t border-[rgba(255,255,255,0.08)] px-4 py-3 flex items-center justify-between shadow-[0_-10px_40px_rgba(0,0,0,0.5)] safe-area-bottom">
          <div className="flex-1">
             {course.is_free ? (
                <div className="font-sans font-bold text-[18px] text-white">Free</div>
             ) : (!course.is_free && course.discounted_price < course.price) ? (
                <div className="flex items-center gap-2">
                  <span className="font-sans font-bold text-[18px] text-white">₹{course.discounted_price}</span>
                  <span className="text-[12px] text-[#48484A] line-through">₹{course.price}</span>
                </div>
             ) : (
                <div className="font-sans font-bold text-[18px] text-white">₹{course.price}</div>
             )}
          </div>
          ...
          <div className="flex items-center gap-3">
             <Link href="#enroll" 
                onClick={(e: React.MouseEvent) => {
                  e.preventDefault();
                  window.scrollTo({ top: 0, behavior: 'smooth' }); // In mobile, they can check top card or just go to /login if not authed. The best way is to render the actual sidebar layout logic in mobile if we had it, but for simplicity: we'll route to course detail
                }}
             >
               {enrollment ? (
                 <div className="bg-[#28C840]/10 text-[#28C840] border border-[#28C840]/20 px-6 py-2.5 rounded-full font-semibold text-[14px]">
                   Continue
                 </div>
               ) : (
                 <div className="bg-white text-[#0A0A0A] px-6 py-2.5 rounded-full font-bold text-[14px]">
                   Enroll Now
                 </div>
               )}
             </Link>
